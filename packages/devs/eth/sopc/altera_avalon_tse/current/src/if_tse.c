...
#define DEBUG 9
int getPHYSpeed(np_tse_mac *pmac);
...
static void
tse_deliver(struct eth_drv_sc *sc)
{
    struct tse_priv_data *cpd =	(struct tse_priv_data *)sc->driver_private;
	...
	cpd->bytesReceived = alt_avalon_sgdma_descpt_bytes_xfered( (alt_sgdma_descriptor *)cpd->rx_sgdma.descriptor_base) - 2;
...
static int
tse_int_vector(struct eth_drv_sc *sc)
{
    struct tse_priv_data *cpd =	(struct tse_priv_data *)sc->driver_private;
...
static bool 
tse_init(struct cyg_netdevtab_entry *tab)
{
    ...
    diag_printf("[tse_mac_init]\n");
  
    /* Get the Rx and Tx SGDMA addresses */
    cpd->rx_sgdma.base 				= SGDMA_RX_BASE;
    cpd->rx_sgdma.irq  				= SGDMA_RX_IRQ;
	cpd->rx_sgdma.descriptor_base	= &((alt_sgdma_descriptor *)DESCRIPTOR_MEMORY_BASE)[ ALTERA_TSE_FIRST_RX_SGDMA_DESC_OFST];
	...
	cpd->tx_sgdma.descriptor_base	= &((alt_sgdma_descriptor *)DESCRIPTOR_MEMORY_BASE)[ ALTERA_TSE_FIRST_TX_SGDMA_DESC_OFST];
	cpd->tx_sgdma.isr               =  (cyg_ISR_t *)0; //tse_txd_isr	   ; // isr
    cpd->tx_sgdma.dsr				=  (cyg_DSR_t *)0;
...
//    duplex = result & 0x01;
    ...
    i = 0;
    while(IORD_ALTERA_TSEMAC_CMD_CONFIG(cpd->base) & ALTERA_TSEMAC_CMD_SW_RESET_MSK) 
    {
    	if( i++ > 10000 ) 
    	{
       		break;
	...
	// let us know if the mac did not complete reset in the in the time alloted
   	if( i >= 10000) 
   	{
    	printf("TSEMAC SW reset bit never cleared!\n");
    }

	// read the command/comfig register and verify that the tx/rx/enable bits are cleared
...
//
static void
tse_start(struct eth_drv_sc *sc, unsigned char *enaddr, int flags)
{
    struct tse_priv_data *cpd = (struct tse_priv_data *)sc->driver_private;
	...
	alt_sgdma_descriptor *desc_base = cpd->rx_sgdma.descriptor_base;
  	...
  	/* enable MAC */
   	dat = ALTERA_TSEMAC_CMD_TX_ENA_MSK    | ALTERA_TSEMAC_CMD_RX_ENA_MSK   | mmac_cc_RX_ERR_DISCARD_mask |
#if ENABLE_PHY_LOOPBACK
        ALTERA_TSEMAC_CMD_PROMIS_EN_MSK   | ALTERA_TSEMAC_CMD_LOOPBACK_MSK |     // promiscuous mode
#endif
//        ALTERA_TSEMAC_CMD_PAD_EN_MSK       |
        ALTERA_TSEMAC_CMD_TX_ADDR_INS_MSK | ALTERA_TSEMAC_CMD_RX_ERR_DISC_MSK;  /* automatically discard frames with CRC errors */
...
//
static int
tse_control(struct eth_drv_sc *sc, unsigned long key, void *data, int data_length)
{
    DEBUG_FUNCTION();
	printf("tse_control\n");
    return 1;
...
//
static int
tse_can_send(struct eth_drv_sc *sc)
{
    DEBUG_FUNCTION();
	...
	// Make sure DMA controller is not busy from a former command	and TX is able to accept data
  	return (( alt_avalon_sgdma_status( &cpd->tx_sgdma) & ALTERA_AVALON_SGDMA_STATUS_BUSY_MSK) == 0);
}

//
// This routine is called to send data to the hardware.
static void 
tse_send(struct eth_drv_sc *sc, struct eth_drv_sg *sg_list, int sg_len, int total_len, unsigned long key)
{
    struct tse_priv_data *cpd = (struct tse_priv_data *)sc->driver_private;
    int i, len, plen, tcr;
...
static void
tse_TxEvent(struct eth_drv_sc *sc, int stat)
{
    DEBUG_FUNCTION();
#if 0
...
//
static void
tse_RxEvent(struct eth_drv_sc *sc)
{
    struct tse_priv_data *cpd = (struct tse_priv_data *)sc->driver_private;
...
//
static void
tse_recv(struct eth_drv_sc *sc, struct eth_drv_sg *sg_list, int sg_len)
{
    struct tse_priv_data *cpd = (struct tse_priv_data *)sc->driver_private;
...
int getPHYSpeed(np_tse_mac *pmac) {
    ...
    DEBUG_FUNCTION();
        ...
        if (0 == TSE_MAC_USE_MDIO)
        {
            ...
            printf("[triple_speed_ethernet_init] Speed is 1000 is %d\t Full Duplex is %d\n", is1000, duplex);
            return result;        
...
//                    dprintf("[phyID] 0x%x %x %x\n",phyadd, phyid, phyid2);
           ...
           if((phyid == (NTLPHY_ID >> 16)) && (phyid2 == (NTLPHY_ID & 0xFFFF))) { 
            ...
            if(isKnown == 0){
                ...
                if((phyid == (NTL848PHY_ID >> 16)) && (phyid2 == (NTL848PHY_ID & 0xFFFF))) { 
            ...
            if( isKnown ) {                                
                ...
                } else if((phyid == (NTL848PHY_ID >> 16)) && (phyid2 == (NTL848PHY_ID & 0xFFFF))) { 
 ...
 // for simulation purpose, default to gigabit mode
  ...
  result = ((is1000 & 0x01) << 1 )|  (duplex & 0x01); 
  diag_printf("[triple_speed_ethernet_init] Speed is 1000 is %d\t Full Duplex is %d\n", is1000, duplex);
